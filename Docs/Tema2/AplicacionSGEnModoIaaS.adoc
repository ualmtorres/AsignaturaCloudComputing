...
## Despliegue en cloud privado OpenStack

El cloud privado OpenStack ofrece Infraestructura como servicio dentro de la red de la UAL. Para conexiones desde el exterior es necesaria una conexión VPN.

### Requisitos previos

* Cuenta creada en el cloud privado OpenStack usado en la asignatura.
* Proyecto OpenStack con la infraestructura básica de red configurada. https://ualmtorres.github.io/OpenStackSTIC/#truenetworking[(Tutorial Configuración de infraestructura básica de red]. En el cloud privado la red externa se denomina `externa`, `public`, o similar.
...
+
. En el paso `Origen` se indica cuál es la imagen que se toma de base para crear la instancia o máquina virtual. En este ejemplo seleccionar en el desplegable `Seleccionar Origen de arranque` el valor `Imagen`. En la opción `Crear nuevo volumen` seleccionar `No`. Seleccionar la imagen de Ubuntu 24.04 que hayas disponible. *El script de instalación, configuración e inicialización de una base de datos que se proporciona está probado en Ubuntu 24.04 LTS*
. En el paso `Sabor` se selecciona el tamaño que tendrá la instancia en términos de CPU, RAM y disco. Seleccionar el sabor `small` o `m1.small` que proporciona 1 vCPU, 2 GB de RAM y 20 GB de disco, que es suficiente para este ejemplo.
...
apt-get -y install mysql-server-8.0 <3> 
...
sed -i 's/^\(bind-address\s*=\s*127\.0\.0\.1\)/#\1/' /etc/mysql/mysql.conf.d/mysqld.cnf <5>
sed -i 's/^\(mysqlx-bind-address\s*=\s*127\.0\.0\.1\)/#\1/' /etc/mysql/mysql.conf.d/mysqld.cnf
service mysql restart <6>
mysql -u root -psecret --execute="CREATE USER IF NOT EXISTS 'root'@'%' IDENTIFIED BY 'secret'; GRANT ALL PRIVILEGES ON SG.* TO 'root'@'%'; FLUSH PRIVILEGES;" <7>
...
<6> Reinicio del servicio MySQL para aplicar los cambios
<7> Permitir el acceso al usuario `root` a la base de datos SG desde cualquier sitio
...
sudo apt-get install -y php8.3 libapache2-mod-php8.3 php8.3-mysql php8.3-mcrypt <3>
sed -i 's/;extension=mysqli/extension=mysqli/g' /etc/php/8.3/apache2/php.ini <4>
...
. En la sección `Disco de arranque` elegir una imagen Debian y mantener el disco en 10 GB de tamaño.

+
image::images/GoogleCloudCrearSGApp.png[]

. En el menú `Redes` seleccionar la casilla de verificación `Permitir el tráfico HTTP`.

+
image::images/GoogleCloudPermitirHTTP.png[]


. En el menú `Avanzado`, en el apartado `Automatización` se puede pasar un script de inicialización que configure la instancia de forma automática tras su creación. En este caso hay que incluir el script siguiente:

[NOTE]
====
A fecha de este tutorial la versión de PHP disponible en los repositorios de Debian es la 8.2. En el futuro puede que haya versiones más recientes.
====
...
apt-get update <1>
apt-get install -y apache2 <2>
apt-get install -y php8.2 libapache2-mod-php8.2 php8.2-mysql php8.2-mcrypt <3>
sed -i 's/;extension=mysqli/extension=mysqli/g' /etc/php/8.2/apache2/php.ini <4>
