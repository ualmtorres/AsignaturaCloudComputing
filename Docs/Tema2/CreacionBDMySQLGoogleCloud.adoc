...
. En el Menú de navegación ir a la categoría `Cloud SQL` y seleccionar `Instancias`. Esto permite crear bases de datos MySQL, PostgreSQL y SQL Server totalmente gestionadas. Google Cloud se encargará de las tareas de replicación, actualización de parches, y demás.
...
.. Desplegar `Configuración de la máquina`. Como es para una prueba, elegir una máquina pequeña con 1 vCPU y 3.75 GB de RAM. 
.. En `Almacenamiento` dejar `HDD` con una capacidad de almacenamiento de `10 GB`. 
.. Desactivar la opción `Habilitar los aumentos de almacenamiento automáticos` para evitar el crecimiento del almacenamiento.
.. Dejar las opciones predeterminadas de `Protección de datos`.
...
+
. En `Redes`, pulsar sobre `Agregar una red`
...
. Lee los riesgos de seguridad asociados a esta configuración y confirma marcando la casilla de verificación.
...
Cloud Storage proporciona almacenamiento de objetos, una forma de almacenar cualquier tipo de archivos a los que se proporciona una URL que permite su acceso universal. Los archivos son organizados en depósitos o _buckets_.

. En el menú `Cloud Storage` seleccionar `Buckets`. Aparecerá el Navegador de Storage mostrando una lista vacía de _buckets_

+
image::images/StorageInicial.png[]

. Seleccionar `Crear bucket`. En el cuadro de diálogo asignar un nombre al bucket. Debe tratarse de un nombre único ya que se usará para componer la URL de los objetos (archivos) que contenga. Revisar el resto de opciones y crear el bucket.
...
+
. Abrir Cloud Shell para descargar el script SQL y copiarlo al bucket.
...
+
. Seleccionar el bucket (p.e. `cloud-computing-2026-mtorres`) y el archivo `init.sql` del bucket    
...
. Si no se modificada nada en `SO y almacenamiento`, se usará un disco de 10 GB con Debian 12 bookworm como sistema operativo.
...
image::images/InstanciaCreada.png[]
